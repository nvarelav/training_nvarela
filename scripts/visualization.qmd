...
```{r}
#clean version with the use of the janitor package
...
ggplot(mapping = aes(x = species, y = escapement)) +
  geom_boxplot()
```
### Making nice tables
...
```
```{r}
# Here is an example that uses the General Bathymetric Map of the Oceans (GEBCO) WMS tiles (addWMSTiles)
leaflet(locations) %>%
    addWMSTiles(
